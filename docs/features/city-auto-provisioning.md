...
| `lib/mock-data.ts` | UI mock data including `SelectedCity` with slugs | `Google Places predictions`, `Google Places predictions` | None | None |
| `components/CityPickerModal.tsx` | City selection UI for trip creation | `CityPickerModal` component | `lib/mock-data` | None |
...
**CityPickerModal** (`components/CityPickerModal.tsx`): Presents cities from `Google Places predictions` and `Google Places predictions`. Each `SelectedCity` carries a `slug` field that maps to the registry.
...
The `CityPickerModal` sources its city list from `Google Places predictions` and `Google Places predictions` in `lib/mock-data.ts`, not from `lib/city-registry.ts`. The slugs in mock data (`san-francisco`, `london`, `tokyo`, `paris`, `new-york`, `barcelona`) are a subset of registry slugs, so they will resolve correctly. But any future city added to mock data without a corresponding registry entry will fail silently at provisioning.
